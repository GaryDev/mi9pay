...
namespace Mi9Pay.Web.ActionFilters
{
    ...
    {
        private static readonly string _passPhase = AppConfig.PasswordPhase;
        ...
        protected virtual BasicAuthenticationIdentity FetchAuthHeader(HttpActionContext filterContext)
        {
            ...
            try
            {
                authHeaderValue = AESHelper.AESDecrypt(authHeaderValue, _passPhase);
                var credentials = authHeaderValue.Split(':');
                identity = credentials.Length < 2 ? null : new BasicAuthenticationIdentity(credentials[0], credentials[1]);
            }
            catch (Exception)
            {
                identity = null;
