    ...
    public sealed class NLogHelper : ITraceWriter
    {
        ...
        public void Trace(HttpRequestMessage request, string category, TraceLevel level, Action<TraceRecord> traceAction)
        {
            if (level != TraceLevel.Off)
            {
                ...
                {
                    string jsonParameter = new JavaScriptSerializer().Serialize(traceAction.Target);
                    category = category + Environment.NewLine + "Action Parameters : " + jsonParameter;
                }
        ...
        private void Log(TraceRecord record)
        {
            ...
            string content = Convert.ToString(message) + Environment.NewLine;
            Logger[record.Level].Invoke(content);
        }
        #endregion
    }
}